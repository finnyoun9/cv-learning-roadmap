...
int main(void)
{
	...
	while (1)
	{
		...
		{
			uint8_t RecvResult = NRF24L01_Receive();	//获取接收结果
			if (RecvResult == 1)			//如果收到回传数据
			{
				...
				{
					OLED_Printf(0, 16, OLED_8X16, "Rx:ID=%02X", ID);	//收到其他ID的数据包
				}
			}
			else if (RecvResult != 0)
			{
				OLED_Printf(0, 16, OLED_8X16, "RxErr:%d", RecvResult);	//接收出错
			}
		}
