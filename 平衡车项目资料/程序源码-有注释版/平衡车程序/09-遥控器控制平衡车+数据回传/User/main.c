...
uint8_t LastRecvID;		//最近一次收到的NRF数据包ID
uint8_t LastSendResult;	//最近一次回传发送结果
...
int main(void)
{
	...
	while (1)
	{
		...
		OLED_Printf(56, 56, OLED_6X8, "Offset:%02.0f", AnglePID.OutOffset);		//显示输出偏移值
			OLED_Printf(56, 8, OLED_6X8, "RID:%02X S:%d", LastRecvID, LastSendResult);	//显示收到的ID和发送结果
			...
			if (ID == 0x00 || ID == 0x01)
			{
				if (ID == 0x01)		//如果ID是0x01，进行数据回传
				{
					...
					*(float *)&NRF24L01_TxPacket[12] = RightSpeed;	//回传数据包字节12 13 14 15，规定为RightSpeed
					...
					LastSendResult = NRF24L01_Send();		//NRF24L01发送回传数据包，保存结果
				}
